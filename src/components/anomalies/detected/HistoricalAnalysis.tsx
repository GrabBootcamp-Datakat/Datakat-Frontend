...
import { ChartDataPoint, EventDataPoint, TimeDataPoint } from '../types';
...
// === COMPONENT: Pie Chart ===
function ComponentPieChart({ data }: { data: ChartDataPoint[] }) {
  return (
...
function LevelBarChart({ data }: { data: ChartDataPoint[] }) {
  return (
...
function EventBarChart({ data }: { data: EventDataPoint[] }) {
  return (
          ...
          <Tooltip
            content={({ active, payload }) => {
              if (active && payload?.length) {
                const d = payload[0].payload as EventDataPoint;
                return (
                  <div
                    style={{
                      background: '#fff',
...
function TimeLineChart({ data }: { data: TimeDataPoint[] }) {
  return (
